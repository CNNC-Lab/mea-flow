...
def identify_discriminative_features(
    condition_results: Dict[str, Dict[str, Any]],
    feature_type: str = 'population_statistics'
) -> pd.DataFrame:
    """
    ...
    feature_type : str
        Type of features to analyze ('population_statistics', 'embedding_quality')
    ...
    if not SKLEARN_AVAILABLE:
        warnings.warn("scikit-learn required for feature analysis")
        return pd.DataFrame()
    
    # Collect features
    if feature_type == 'population_statistics':
        feature_data = []
        labels = []
        ...
        for condition_name, results in condition_results.items():
            if 'population_statistics' in results:
                ...
                # Convert to list, excluding non-numeric values
                features = [v for v in stats.values() if isinstance(v, (int, float, np.number)) and not np.isnan(v)]
                if len(features) > 0:
        ...
        min_length = min(len(f) for f in feature_data)
        feature_data = [f[:min_length] for f in feature_data]
        
        X = np.array(feature_data)
        y = labels
        
        # Get feature names
        sample_stats = list(condition_results.values())[0]['population_statistics']
        feature_names = [k for k, v in sample_stats.items() 
                        if isinstance(v, (int, float, np.number)) and not np.isnan(v)][:min_length]
        
    else:
        return pd.DataFrame({'error': f'Unknown feature type: {feature_type}'})
